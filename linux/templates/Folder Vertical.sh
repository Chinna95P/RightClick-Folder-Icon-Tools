...
FolderNameLong_characters_limit="40"
...
#                Images Source
#========================================================
foldervertical_side="$APP_DIR/images/foldervertical-side.png"
foldervertical_sidefx="$APP_DIR/images/foldervertical-sidefx.png"
foldervertical_sideshadow="$APP_DIR/images/foldervertical-sideshadow.png"
foldervertical_main="$APP_DIR/images/foldervertical-main.png"
foldervertical_mainfx="$APP_DIR/images/foldervertical-mainfx.png"
...
LAYER_POSTER_MAIN=( \( "$inputfile" -scale 346x490\! -gravity Northwest -geometry +70+14 "$foldervertical_main" \) -compose over -composite \( "$foldervertical_mainfx" -scale 512x512\! \) -compose over -composite )

LAYER_POSTER_SIDE=( \( "$inputfile" -scale 512x512\! -blur 0x19 "$foldervertical_side" \) -compose over -composite \( "$foldervertical_sidefx" -scale 512x512\! \) -compose over -composite )

LAYER_POSTER_SIDE_SHADOW=( \( "$foldervertical_sideshadow" -scale 512x512\! \) -compose over -composite )
...
if [[ "$display_movieinfo" == "yes" ]]; then
    ...
    if $has_nfo; then
        ...
        if [[ "$show_Rating" == "yes" ]]; then
            LAYER_STAR_IMAGE=( \( "$star_image" -scale 88x84\! -gravity Northwest -geometry +46+417 \( +clone -background BLACK -shadow 0x1.2+4+6 \) +swap -background none -layers merge -extent 512x512 \) -compose Over -composite )
            if [[ -n "$rating" ]]; then
                LAYER_RATING=( \( -font "$APP_DIR/resources/ANGIE-BOLD.TTF" -fill "rgba(0,0,0,0.9)" -density 400 -pointsize 6 -kerning 0 label:"$rating" -gravity Northwest -geometry +58+440 \( +clone -background ORANGE -shadow 30x1.2+2+2 \) +swap -background none -layers merge \( +clone -background YELLOW -shadow 30x1.2-2-2 \) +swap -background none -layers merge \( +clone -background ORANGE -shadow 30x1.2-2+2 \) +swap -background none -layers merge \( +clone -background ORANGE -shadow 30x1.2+2-2 \) +swap -background none -layers merge \) -compose Over -composite )
            fi
        fi
    fi
fi
...
if [[ "$use_Logo_instead_FolderName" == "yes" ]] && [[ "$custom_FolderName_HaveTheLogo" != "yes" ]]; then
    ...
    if [[ -n "$Logo" ]]; then
        LAYER_LOGO_IMAGE=( \( "$Logo" -trim +repage -scale 160x50^ -background none -gravity center -geometry +190-114 -rotate 90 \) -compose Over -composite )
    fi
fi
...
if [[ "$display_FolderName" == "yes" ]] && [[ ${#LAYER_LOGO_IMAGE[@]} -eq 0 ]]; then
    ...
    FolNamCenter=( -gravity center -geometry +200-114 )
    FolNamLeft=( -gravity Northwest -geometry +398+32 )
    ...
    LAYER_FOLDER_NAME_SHORT=( \( -font Arial-Bold -fill "$FolderName_Font_Color" -density 400 -pointsize 5 -kerning 0 "${FolNamPos[@]}" -background none label:"$FolNamShort" \( +clone -background BLACK -shadow 10x5+0.6+0.6 \) +swap -background none -layers merge \( +clone -background BLACK -shadow 10x5-0.6-0.6 \) +swap -background none -layers merge \( +clone -background BLACK -shadow 10x5-0.6+0.6 \) +swap -background none -layers merge \( +clone -background BLACK -shadow 10x5+0.6-0.6 \) +swap -background none -layers merge -rotate 90 \) -composite )

    if [[ ${#foldername} -gt $FolNamShortLimit ]] && [[ "$FolderNameLong_characters_limit" != "0" ]]; then
        LAYER_FOLDER_NAME_LONG=( \( -font Arial-Bold -fill "$FolderName_Font_Color" -density 400 -pointsize 3.1 -kerning 2 -gravity Northwest -geometry +375+5 -background none label:"$FolNamLong" \( +clone -background BLACK -shadow 10x5+0.2+0.2 \) +swap -background none -layers merge \( +clone -background BLACK -shadow 10x5-0.2-0.2 \) +swap -background none -layers merge \( +clone -background BLACK -shadow 10x5-0.2+0.2 \) +swap -background none -layers merge \( +clone -background BLACK -shadow 10x5+0.2-0.2 \) +swap -background none -layers merge -rotate 90 \) -composite )
    fi
fi
